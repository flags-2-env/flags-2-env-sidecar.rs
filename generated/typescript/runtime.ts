...
export function loadFrom(lookup: (key: string) => string | undefined): SidecarEnvValues {
  return {
    bind: nonEmpty(lookup("FLAGS_2_ENV_SIDECAR_BIND")) ?? "127.0.0.1:9090",
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const bind = pick(["FLAGS_2_ENV_SIDECAR_BIND"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "127.0.0.1:9090");
  if (bind !== undefined) out["FLAGS_2_ENV_SIDECAR_BIND"] = bind;
  return out;
}
